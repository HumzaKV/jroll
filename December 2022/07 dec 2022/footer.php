...
<script>
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).on('click', 'ul li.buynow a', function(e) {
            e.preventDefault()
            const url = $(location).attr("origin")+'/?add-to-cart=697&quantity=1';
            $.ajax({
                type: 'GET',
                url:url,
...
if ( is_user_logged_in() && home_url($_SERVER['REQUEST_URI']) == home_url('/jrollx10-2/my-account') ) 
{
...
<div class="section">
 <div class="container">
     <h1>Welcome, '.$current_user->user_firstname.'!</h1>
 </div>
</div>
';
ob_get_clean();
