...
//Promotion strip Carousel
    ...
    // open cart on 
//     jQuery(document).on("click","#menu-item-3625",function() {
//     	jQuery('.cart .xoo-wsc-cart-trigger').click();
// });
		jQuery(document).on("click",".web-btn",function() {
    		jQuery('.cart .xoo-wsc-cart-trigger').click();
});

        jQuery(document).on("click","ul li.buynow a",function() {
   		 	jQuery('.cart .xoo-wsc-cart-trigger').click();
});

// Reaaranging fields of wholesale Regestration
jQuery('.woocommerce-Input').closest('form.woocommerce-form-register').find('p.woocommerce-form-row').eq( 0 ).html('');
jQuery('.woocommerce-Input').closest('form.woocommerce-form-register').find('p.woocommerce-form-row').eq( 1 ).html('');
jQuery('<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide"><label for="reg_email">Email address&nbsp;<span class="required">*</span></label><input type="email" class="woocommerce-Input woocommerce-Input--text input-text" name="email" id="reg_email" autocomplete="email" value="">			</p><p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide"> <label for="reg_password">Password&nbsp;<span class="required">*</span></label> <span class="password-input"><input type="password" class="woocommerce-Input woocommerce-Input--text input-text" name="password" id="reg_password" autocomplete="new-password"><span class="show-password-input"></span></span> </p>').insertAfter(jQuery('#b2bking_field_3692').closest('p.woocommerce-form-row'));

// Changing Login Email label
jQuery('.woocommerce-form-login').find('p.woocommerce-form-row').find('label').first().text('Email Address');
...
function myfunc_registration_save() {
	...
	{
		wp_redirect( 'https://staging2.jroll.com/wholesalety/' ); 
		exit;
	}
